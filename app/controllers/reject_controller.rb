class RejectController < ApplicationController
        ...
        if power(T_K3_Auth, "t_reject_auth")#T_Reject_Auth.find_by_name(params[:name])
            ...
            if djzt&&(""!=djzt)
                case djzt
                    ...
                    when "待生管负责人审核"
                        sqlDjzt += " and 关闭标志 = 0 and 品质负责人 is not null and 生管负责人 is null";
                    when "待财务负责人审核"
                        sqlDjzt += " and 关闭标志 = 0 and 生管负责人 is not null and 财务负责人 is null";
                    when "待分管副总审核"
            ...
            if ""!=cpdm 
                if bfsl != 0
                    ...
                    if "false_more" == bfjetotal
                        ...
                        sqlInsert3 = "insert into t_RejCheckFlu(FRejBillNo,RejAmount_1,RejDate) values('"+djbh+"','"+bfjetotal.to_s+"','"+rejDate+"')";
                        a = inputs.length/6
    ...
    def sh
    	...
    	when "bm"
    		if dept==sql&&power(T_Reject_Auth, "dept_auth")
                ...
                if djbhInsOrUpd&&(""!=djbhInsOrUpd)
                    a="update t_RejCheckFlu set RejDeptManager = '"+@user.name+"' , RejDeptDate = '"+rejDeptDate+"' where FRejBillNo = '"+djbh+"'"
                    if conn.update(a)
                        render :text => "审核成功"
                    else
                        render :text => "审核失败"
                    ...
                    a="insert into t_RejCheckFlu (FRejBillNo,RejDeptManager,RejDate,rejDeptDate) values('"+djbh+"','"+@user.name+"','"+rejDeptDate+"','"+rejDeptDate+"')"
                    if conn.insert(a)
                        render :text => "审核失败"
                    else
                        render :text => "审核成功"
    	...
    	when "sg"
    		...
    		if power(T_Reject_Auth, "sg_auth")
    			a="update t_RejCheckFlu set ProManager = '"+@user.name+"',ProManagerNote = '"+params[:note]+"', ProDate = '"+proDate+"' where FRejBillNo = '"+djbh+"'"
    			if conn.update(a)
                    render :text => "审核成功"
                else
                    render :text => "审核失败"
    	...
    	when "cw"
    		if power(T_Reject_Auth, "cw_auth")
    			...
    			a="update t_RejCheckFlu set FinManager = '"+@user.name+"',FinManagerNote = '"+params[:note]+"',RejAmount_1 = '"+params[:fin]+"', FinDate = '"+finDate+"' where FRejBillNo = '"+djbh+"'"
    			if conn.update(a)
                    render :text => "审核成功"
                else
                    render :text => "审核失败"
    	...
    	when "fgfz"
    		if power(T_Reject_Auth, "fz_auth")
    			...
    			a="update t_RejCheckFlu set ViseManager = '"+@user.name+"',ViseManagerNote = '"+params[:note]+"', ViseDate = '"+viseDate+"' where FRejBillNo = '"+djbh+"'"
    			if conn.update(a)
                    render :text => "审核成功"
                else
                    render :text => "审核失败"
    	...
    	when "ck"
    		if power(T_Reject_Auth, "cgy_auth")
    			...
    			a="update t_RejCheckFlu set WarehouseAdmin = '"+@user.name+"', WarehouseDate = '"+warehouseDate+"' where FRejBillNo = '"+djbh+"'"
    			if conn.update(a)
                    render :text => "审核成功"
                else
                    render :text => "审核失败"
    ...
    def sgPlsh
        ...
        if power(T_Reject_Auth, "sg_auth")
            ...
            conn = ActiveRecord::Base.connection()
            sql = "update t_RejCheckFlu set ProManager = '"+@user.name+"',ProManagerNote = '"+params[:sgyj]+"', ProDate = '"+proDate+"' where FRejBillNo in ("+fbillno+")";
            if conn.update(sql)
                render :text => "审核成功"
            else
                render :text => "审核失败"
    ...
    def cwPlsh
        ...
        if power(T_Reject_Auth, "cw_auth")
            ...
            conn = ActiveRecord::Base.connection()
            sql = "update t_RejCheckFlu set FinManager = '"+@user.name+"',FinManagerNote = '"+params[:cwyj]+"', FinDate = '"+finDate+"' where FRejBillNo in ("+fbillno+")";
            if conn.update(sql)
                render :text => "审核成功"
            else
                render :text => "审核失败"
    ...
    def plsh
        ...
        if power(T_Reject_Auth, "fz_auth")
            ...
            conn = ActiveRecord::Base.connection()
            sql = "update t_RejCheckFlu set ViseManager = '"+@user.name+"',ViseManagerNote = '"+params[:fgfzyj]+"', ViseDate = '"+viseDate+"' where FRejBillNo in ("+fbillno+")";
            if conn.update(sql)
                render :text => "审核成功"
            else
                render :text => "审核失败"
    ...
    def meSh(meManager, meManagerNote, djbh, auth)
        if power(T_Reject_Auth, auth)
            ...
            conn = ActiveRecord::Base.connection()
            a="update t_RejCheckFlu set MEManager = '"+meManager+"',MEManagerNote = '"+meManagerNote+"',MeDate = '"+meDate+"' where FRejBillNo = '"+djbh+"'"
            if conn.update(a)
                render :text => "审核成功"
            else
                render :text => "审核失败"
    ...
    def pzSh(qltManager, qltManagerNote, djbh, auth)
        ...
        if power(T_Reject_Auth, auth)
            conn = ActiveRecord::Base.connection()
            a="update t_RejCheckFlu set QltManager = '"+qltManager+"',QltManagerNote = '"+qltManagerNote+"', QltDate = '"+qltDate+"' where FRejBillNo = '"+djbh+"'"
            if conn.update(a)
                render :text => "审核成功"
            else
                render :text => "审核失败"
