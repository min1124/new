                ...
                Cust_Fnumber.transaction do
                    sheet.each  do |row|
                        if row[0]=="来源"
                            ...
                            custFnumber.customer_number = row[row1.index("Customer Number")]
                            custFnumber.product_model = row[row1.index("Product Model")]
                            custFnumber.fnumber = row[row1.index("BOM code")]
                            custFnumber.customer_code = row[row1.index("Customer Code")]
                            custFnumber.product_line = row[row1.index("Product Line")]
                            custFnumber.customer_name = row[row1.index("The customer name")]            
                            custFnumber.product_status = row[row1.index("Product Status")]
                            custFnumber.shipping_or_not = row[row1.index("shipping or not")]
                            custFnumber.update_date = row[row1.index("新增日期")]
                            custFnumber.F11 = row[row1.index("备注说明")]
                            custFnumber.save!
                        end
                    end
                end
